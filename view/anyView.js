 ...
 *                                   Default: `this.model`.
 * @param {Object}  params.data      The data to display / display from. If given, overrides data in `model`.
 *                                   Default: `model.data` if `model` is set, null otherwise.
 * @param {string}  params.type      The type of the data to display. If not given, finds value from `data`.
 *                                   Default: null.
 * @param {string}  params.mode      The mode in which to display data ("list", "item" or "head").
 *                                   If not given, finds value from `data` and `type`.
 *                                   Default: "head".
 * @param {Object}  params.link_data The data on the level above `data`.
 *                                   Ignored if `model` is given, in which case `model.parent.data` is used.
 *                                   Default: null.
 * @param {string}  params.link_id   The id in `link_data` where `data` may be found (`link_data[link_id] == data`).
 *                                   Ignored if `model` is given, in which case `model.parent.id` is used.
 *                                   Default: null.
 * @param {string}  params.link_type The type of the data on the level above.
...
$.any.anyView.prototype.refresh = function (params)
{
  ...
  let type      = params && params.type      ? params.type      : this._findType(data);
  let mode      = params && params.mode      ? params.mode      : this._findMode(data,null,type);
  let link_data = params && params.link_data ? params.link_data : model ? model.parent ? model.parent.data : null : null;
  let link_id   = params && params.link_id   ? params.link_id   : model ? model.parent ? model.parent.id   : ""   : "";
  let link_type = params && params.link_type ? params.link_type : model ? model.parent ? model.parent.type : ""   : "";
  ...
  if (data) {
    if (Object.size(data) !== 0) {
      ...
      for (let id in data) {
        if (data.hasOwnProperty(id) && !id.startsWith("grouping") && !(id.startsWith("link-") && !this.options.showSublists)) {
          if (view) {
    ...
    this.refreshNoData(params); // We might need to do something if no data
...
$.any.anyView.prototype.refreshOne = function (params)
{
  ...
  if (data) {
    ...
    if (subdata && Object.size(subdata) > 0) {
      let subtype = this._findType(data,id,type);
      let submode = this._findMode(data,id,type);
...
$.any.anyView.prototype.refreshToolbarForView = function (params)
{
  ...
  let mode      = params.mode;
  let id_str    = params.id_str;
  let link_data = params.link_data;
  let link_id   = params.link_id;
  let link_type = params.link_type;

  if (!parent || !parent.length || !type || mode != "item" ||
      link_type !== "group" || link_id === "nogroup" || link_id === "unknown" ||
      (type == "group" && link_type == "group"))
    return null;
  ...
  for (let filter_id in filter) {
    if (filter.hasOwnProperty(filter_id) && filter_id != type+"_ingress") {
      ...
      if (filter_key && filter_key.DISPLAY) {
        ...
        let str = this.getCellEntryStr(this.model,data[id],lists,mode,row_id_str,filter,filter_id,filter_key,edit,td);
...
$.any.anyView.prototype.refreshTableDataIngress = function (params)
{
  ...
  if (filter_key && data[id] && data[id][filter_id]) {
    let lists = filter_key.TYPE == "list" && data[id] && data[id].data ? data[id].data["link-"+filter_key.LIST] : null;
    let str = this.getCellEntryStr(this.model,data[id],lists,mode,row_id_str,filter,filter_id,filter_key,edit);
...
$.any.anyView.prototype.refreshTableDataItemCells = function (params)
{
  ...
  tr.append(td3);
  let lists = filter_key.TYPE == "list" && data[id] && data[id].data ? data[id].data["link-"+filter_key.LIST] : null;
  let str = this.getCellEntryStr(this.model,data[id],lists,mode,row_id_str,filter,filter_id,filter_key,edit,td3);
  if (typeof str == "string")
    td3.append(str);
  this.initTableDataCell(td_id,type,mode,data,id,id_str,row_id_str,filter,filter_id,filter_key,edit,n,link_data,link_id);
  return true;
...
$.any.anyView.prototype.initTableDataCell = function (td_id,type,mode,data,id,id_str,row_id_str,filter,filter_id,filter_key,edit,n,link_data,link_id)
{
  if (!filter_key || !td_id)
    return;
...
{
  if (!opt || !opt.parent || !opt.type || !opt.mode)
    return null;
...
//let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  ...
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
  let btn = $("<div id='"+btn_id+"' class='any-tool-closeitem any-tool-button pointer' title='"+tit_str+"'>"+
              //tit_str+
              //btn_str+
...
$.any.anyView.prototype.refreshAddButton = function (opt)
{
  ...
  let parent  = opt.parent;
  let idstr   = opt.id_str;
  let tit_str = i18n.button.buttonAddToList.replace("%%",opt.type);
  let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  let btn_id  = this.id_base+"_"+opt.type+"_"+opt.mode+"_"+idstr+"_new_line_icon";
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
...
$.any.anyView.prototype.refreshSelectButton = function (opt)
{
  ...
  let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  let btn_id  = this.id_base+"_"+opt.type+"_"+opt.mode+"_"+idstr+"_select_icon";
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
...
$.any.anyView.prototype.refreshEditButton = function (opt)
{
  ...
  let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  let btn_id  = this.id_base+"_"+opt.type+"_"+opt.mode+"_"+idstr+"_edit_icon";
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
...
{
  if (!opt || !opt.edit || !this.options.isEditable)
    return null;
  ...
  let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  let btn_id  = this.id_base+"_"+opt.type+"_"+opt.mode+"_"+idstr+"_update_icon";
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
...
$.any.anyView.prototype.refreshDeleteButton = function (opt)
{
  ...
  let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  let btn_id  = this.id_base+"_"+opt.type+"_"+opt.mode+"_"+idstr+"_delete_icon";
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
...
$.any.anyView.prototype.refreshCancelButton = function (opt)
{
  ...
  let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  let btn_id  = this.id_base+"_"+opt.type+"_"+opt.mode+"_"+idstr+"_cancel_icon";
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
...
$.any.anyView.prototype.refreshRemoveButton = function (opt)
{
  ...
  let btn_str = this.options.showButtonLabels ? "<span class='any-button-text'>"+tit_str+"</span>" : "";
  let btn_id  = this.id_base+"_"+opt.type+"_"+opt.mode+"_"+idstr+"_remove_icon";
  if ($("#"+btn_id).length)
    $("#"+btn_id).remove();
...
$.any.anyView.prototype.refreshAddLinkButton = function (opt)
{
  ...
  let con = this.option("menuContext")
            ...
            : this.option("context")
              ? this.option("context")
              : this;
  opt.top_view = this.options.top_view;
  btn.off("click").on("click", opt, $.proxy(fun,con));
  ...
  if (opt.mode == "item" && this.options.showButtonAddLinkItem) {
    ...
    if (window.anyKeydownHandler.indexOf(opt.id_str) == -1) {
      window.addEventListener("keydown", (e) => this.handleEsc(e,opt,this));
      window.anyKeydownHandler.push(opt.id_str);
    }
...
$.any.anyView.prototype.getCreateModelOptions = function(data,id,type)
{
  return {
    ...
    id:            id,
    type:          type,
    parent:        this.model ? this.model               : null,
...
$.any.anyView.prototype.getCellEntryStr = function (model,data_item,data_lists,mode,id_str,filter,filter_id,filter_key,edit,parent)
{
  if (!filter_id || !filter_key)
    return "";
  let val    = data_item[filter_id];
  let link_id = data_item["parent_id"];
  ...
  switch (filter_key.TYPE) {
    case "label":    return this.getLabelStr   (model,mode,val); // Always noneditable
    case "html":     return this.getHtmlStr    (model,mode,val,edit);
    case "textarea": return this.getTextAreaStr(model,mode,val,edit,filter_id,id_str);
    case "text":     return this.getTextStr    (model,mode,val,edit);
    case "password": return this.getPasswordStr(model,mode,val,edit);
    case "link":     return this.getLinkStr    (model,mode,val,edit);
    case "mailto":
    case "email":    return this.getEmailStr   (model,mode,val,edit);
    case "number":   return this.getNumberStr  (model,mode,val,edit);
    case "date":     return this.getDateStr    (model,mode,val,edit);
    case "image":    return this.getImageStr   (model,mode,val,edit,filter_key);
    case "radio":    return this.getRadioStr   (model,mode,val,edit,filter_key,filter_id);
    case "check":    return this.getCheckStr   (model,mode,val,edit,filter_key,filter_id);
    case "select":   return this.getSelectStr  (model,mode,val,edit,filter_key,link_id,data_item["parent_name"]);
    case "function": return this.getFunctionStr(model,mode,val,edit,filter_key,link_id,data_item["parent_name"]);
    case "list":     return this.getListView   (model,mode,val,edit,filter_key,filter_id,filter,id_str,data_lists,parent);
    case "upload":   return this.getUploadStr  (model,mode,val,edit,data_item,filter_id,id_str);
    case "fileview": return this.getFileViewStr(model,mode,val,edit,data_item,filter_id,id_str);
    /* Not used yet
    case "http":
    case "https":    return this.getHttpStr    (model,mode,val,edit);
    case "textspan": return this.getTextspanStr(model,mode,val,edit);
    case "tokenlist":return this.getTokenlist  (model,mode,val,edit);
    */
  }
  if (!val)
    val = "";
...
$.any.anyView.prototype.getHtmlStr = function (model,mode,val,edit)
{
  return val;
}; // getHtmlStr

$.any.anyView.prototype.getLabelStr = function (model,mode,val)
{
...
$.any.anyView.prototype.getTextAreaStr = function (model,mode,val,edit,filter_id,id_str)
{
  if (edit) {
    if (typeof tinyMCE !== "undefined") {
      let nameid = this.id_base+"_"+model.type+"_"+mode+"_"+id_str+"_"+filter_id;
    ...
    return this.getLabelStr(model,mode,val);
}; // getTextAreaStr

$.any.anyView.prototype.getTextStr = function (model,mode,val,edit)
{
  if (edit)
...
$.any.anyView.prototype.getPasswordStr = function (model,mode,val,edit)
{
  if (edit)
    return "<input class='itemEdit itemText password' type='password' value='"+val+"'/>";
  else
    return this.getLabelStr(model,mode,"");
}; // getLabelStr

$.any.anyView.prototype.getLinkStr = function (model,mode,val,edit)
{
  if (edit)
    return this.getTextStr(model,mode,val,edit);
  else
    return "<div class='itemUnedit itemText pointer underline' attr='link'>"+val+"</div>";
}; // getLinkStr

$.any.anyView.prototype.getEmailStr = function (model,mode,val,edit)
{
  if (edit)
    return this.getTextStr(model,mode,val,edit);
  else
...
$.any.anyView.prototype.getNumberStr = function (model,mode,val,edit)
{
  if (edit)
...
$.any.anyView.prototype.getDateStr = function (model,mode,val,edit)
{
  let str = "";
  if (edit) {
    let dateval = "";
...
$.any.anyView.prototype.getFunctionStr = function (model,mode,val,edit,filter_key,link_id,pname)
{
  let func_name = filter_key.FUNCTION;
  if (isFunction(this[func_name])) // Method in view class
    return this[func_name](model,mode,val,edit,link_id);
  if (isFunction(window[func_name])) // Normal function
    return window[func_name](model,mode,val,edit,link_id);
  return ""; // Function not found
}; // getFunctionStr

$.any.anyView.prototype.getImageStr = function (model,mode,val,edit,filter_key)
{
  let image_src = filter_key.IMAGE;
  if (!image_src && filter_key.FUNCTION && typeof window[filter_key.FUNCTION] == "function")
    return this.getFunctionStr(model,mode,val,edit,filter_key);
  return "<div class='itemUnedit'>"+
         "<img class='imageRef pointer' src='"+image_src+"' title='"+val+"' style='box-shadow:none;'>"+
         "</div>";
}; // getImageStr

$.any.anyView.prototype.getSelectStr = function (model,mode,val,edit,filter_key,link_id,pname)
{
  let str  = "";
  let sval = val;
  ...
  if (fval) {
    if (typeof this[fval] === 'function')
      sval = this[fval](model,mode,val,edit,link_id);
    else
...
$.any.anyView.prototype.getRadioStr = function (model,mode,val,edit,filter_key,filter_id)
{
  let str  = "";
  let sval = val;
  ...
  if (fval) {
    if (typeof this[fval] == "function")
      sval = this[fval](model,mode,val,edit);
    else
    if (typeof fval == "object")
      sval = edit ? fval : fval[val];
    else
...
$.any.anyView.prototype.getCheckStr = function (model,mode,val,edit,filter_key,filter_id)
{
  let str = "";
  if (edit) {
    ...
    let the_id      = Number.isInteger(parseInt(model.id)) ? parseInt(model.id) : model.id;
    let id_str      = ""+the_id;
    let it_id       = this.id_base+"_"+model.type+"_"+mode+"_"+id_str+"_"+filter_id+"_check";
...
$.any.anyView.prototype.getListView = function (model,mode,val,edit,filter_key,filter_id,filter,id_str,data_lists,parent)
{
  if (!this.model)
  ...
  let model_str = filter ? filter[filter_id].MODEL : null;
  ...
  let model_opt   = this.getListModelOptions(model.type,model.id,list_type,val);
  let list_model  = new window[m_str](model_opt);
  list_model.data = data_lists ? data_lists.data : null;

  // Create the list view
  let list_view_id = this.id_base+"_"+model.type+"_"+mode+"_"+id_str+"_"+list_type+"_list";
  let view_opt     = this.getListViewOptions(list_model,list_view_id,edit,this);
  let view_str     = filter ? filter[filter_id].VIEW  : null;
  let v_str        = view_str && typeof view_str === "string"
                     ? view_str
                     : view_opt.grouping
  ...
  try {
    let view = new window[v_str](view_opt);
    if (!Object.keys(view).length) {
...
$.any.anyView.prototype.getUploadStr = function (model,mode,val,edit,data_item,filter_id,id_str)
{
  // Shows a clickable label that opens a file select dialog when pressed
  let elem_id = this.id_base+"_"+model.type+"_"+mode+"_"+id_str+"_"+filter_id; // element id
//let name    = data_item[model.type+"_name"];                                 // real file name from user
  let style   = "style='cursor:pointer;'";
  let title   = "title='Select a new file for upload'"; // TODO i18n
  let filter  = this.getFilter(model.type,mode);
...
$.any.anyView.prototype.getFileViewStr = function (model,mode,val,edit,data_item,filter_id,id_str)
{
  let elem_id  = this.id_base+"_"+model.type+"_"+mode+"_"+id_str+"_"+filter_id; // element id
...
/*
$.any.anyView.prototype.getHttpStr = function (model,mode,val,edit,nameid)
{
  if (edit) {
    return this.getTextStr(model.data,model.id,model.type,mode,val,edit);
  }
  else {
    let id_str  = "" + (Number.isInteger(parseInt(model.id)) ? parseInt(model.id) : model.id);
...
$.any.anyView.prototype.getTokenList = function (model,mode,val,edit,nameid)
{
  return "<input id='"+nameid+"' name='"+nameid+"' class='tokenList' type='text' value='"+val+"'/>";
};

$.any.anyView.prototype.getTextspanStr = function (model,mode,val,edit,nameid)
{
  ...
  if (self.model.data) {
    ...
    if (search_view) {
      ...
      //width:       "30em", // TODO! css
        // Sent to okFunction:
        options: {
          data:        self.data, // TODO! self.type if context is view??
          type:        self.type, // TODO! --- " ---
        },
      });
      search_view.refresh();
...
$.any.anyView.prototype.searchSuccessOk = function (opt,parentId,elementId)
{
  // Close dialog and stop spinner
  w3_modaldialog_close(parentId,elementId);
...
$.any.anyView.prototype.addListEntry = function (event)
{
  ...
  if (edit && (new_id || new_id === 0)) {
    ...
    let new_params = {
      ...
      id:        link_id, // TODO! Is this correct?
      type:      type,
      link_data: link_data,
      link_id:   link_id,
      link_type: link_type,
...
$.any.anyView.prototype._addListEntryFromDB = function (context,serverdata,options)
{
  ...
  if (serverdata && typeof serverdata === "object") {
    ...
    if (view) {
      ...
      serverdata.link_data = options.link_data ? options.link_data : null;
      serverdata.link_id   = options.link_id   ? options.link_id   : null;
      serverdata.link_type = options.link_type ? options.link_type : null;
...
$.any.anyView.prototype.createParentDropdownMenu = function (context,serverdata,params)
{
  let self = context ? context : this;
  self.db_last_command = "sea";
  ...
  if (serverdata) {
    ...
    if (serverdata.data) {
      let view = params.context ? params.context : null;
      if (view) {
        let data      = serverdata.data;
        let the_id    = Number.isInteger(parseInt(params.child_id)) ? parseInt(params.child_id) : params.child_id;
        let mode      = params.mode;
        let type_name = params.type+"_name";
        let id_str    = "0_"+the_id;
        let itemsel_id = view.id_base+"_"+params.type+"_"+mode+"_"+id_str+"_parent_id .itemSelect";
        ...
        data = data[params.type] ? data[params.type].data : data;
        $.each(data,function (id,item) {
          if (parseInt(id) != the_id) {
            let sel = parseInt(id) == parseInt(params.parent_id) ? "selected='true'" : "";
...
$.any.anyView.prototype.dbRemoveDialog = function (event)
{
  ...
  if (this.options.confirmRemove && !data[id].is_new) {
    ...
    if (mode == "list") {
      let li_data = this.model.dataSearch({ data: link_data,
                                            id:   link_id,
                                            type: link_type,
                                         });
    ...
    if (parent_id) {
      let hdng = mode == "item"
                 ? i18n.button.buttonRemove
                 : i18n.button.buttonRemoveFromList.replace("%%",type);
      hdng = hdng.replace("##",link_type);
      let mod_opt = {
        parentId:   parent_id,
        elementId:  "",
        heading:    hdng,
...
$.any.anyView.prototype.dbUpdateLinkList = async function (opt,parentId,elementId)
{
  // Close dialog and stop spinner
  w3_modaldialog_close(parentId,elementId);
  this.showMessages("",false);
  ...
  if (this.options && opt && opt.mode == "item")
    this.options.item_opening = true;

  // Update database
  opt.context = this.model;
  if (await !this.model.dbUpdateLinkList(opt))
    return false;
  return true;
}; // dbUpdateLinkList
...
$.any.anyView.prototype.dbDelete = function (opt,parentId,elementId)
{
